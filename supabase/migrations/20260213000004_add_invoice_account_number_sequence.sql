...
DROP FUNCTION IF EXISTS public.next_invoice_account_number(uuid);

CREATE OR REPLACE FUNCTION public.next_invoice_account_number(
  p_tenant_id uuid
)
RETURNS text
LANGUAGE plpgsql
...
BEGIN
  ...
  WHERE tenant_id = p_tenant_id
    AND doc_key = 'invoice_account_number'
...
COMMENT ON FUNCTION public.next_invoice_account_number(uuid) IS
  'Atomic Account # generator for invoices per tenant. Starts at 11120100 and increments by 1.';

GRANT EXECUTE ON FUNCTION public.next_invoice_account_number(uuid) TO authenticated;
